<head>
    <title>觀光牧場</title>
    <meta charset="utf-8">
    <meta name="viewport" content="width=device-width, initial-scale=1">

    <script src="https://code.jquery.com/jquery-3.3.1.slim.min.js"
        integrity="sha384-q8i/X+965DzO0rT7abK41JStQIAqVgRVzpbzo5smXKp4YfRvH+8abtTE1Pi6jizo"
        crossorigin="anonymous"></script>
    <link rel="stylesheet" href="https://stackpath.bootstrapcdn.com/bootstrap/4.3.1/css/bootstrap.min.css"
        integrity="sha384-ggOyR0iXCbMQv3Xipma34MD+dH/1fQ784/j6cY/iJTQUOhcWr7x9JvoRxT2MZw1T" crossorigin="anonymous">
    <script src="https://stackpath.bootstrapcdn.com/bootstrap/4.3.1/js/bootstrap.min.js"
        integrity="sha384-JjSmVgyd0p3pXB1rRibZUAYoIIy6OrQ6VrjIEaFf/nJGzIxFDsf4x0xIM+B07jRM"
        crossorigin="anonymous"></script>
    <script src="https://cdnjs.cloudflare.com/ajax/libs/popper.js/1.14.7/umd/popper.min.js"
        integrity="sha384-UO2eT0CpHqdSJQ6hJty5KVphtPhzWj9WO1clHTMGa3JDZwrnQq4sF86dIHNDz0W1"
        crossorigin="anonymous"></script>
    <link rel="stylesheet" type="text/css" href="../css/shopbutton.css">
</head>

<body>
    <nav class="navbar navbar-expand-md navbar-dark fixed-top bg-info">
        <a class="navbar-brand" href="index.html">觀光牧場</a>
        <button class="navbar-toggler" type="button" data-toggle="collapse" data-target="#navbarCollapse"
            aria-controls="navbarCollapse" aria-expanded="false" aria-label="Toggle navigation">
            <span class="navbar-toggler-icon"></span>
        </button>
        <div class="collapse navbar-collapse" id="navbarCollapse">
            <ul class="navbar-nav mr-auto">
                <li><a class="nav-link" href="feeder.php">動物資訊</a></li>
                <li><a class="nav-link" href="Mdetail.php">原料資訊</a></li>
                <li><a class="nav-link" href="order.php">通知</a></li>
                <li><a class="nav-link" href="logout.php">登出</a></li>
        </div>
    </nav>
    <br><br><br>
<?php session_start(); 
    if($_SESSION['username'] == null){ 
      //header("Location: xxxx.com");
      echo '<meta http-equiv=REFRESH CONTENT=2;url=http://localhost/php_example/main.php>';  
      echo "<script>history.forward()</script>";
      exit(); 
    }
    

    $dbhost = '127.0.0.1';
    $dbuser = 'hj';
    $dbpass = 'test1234';
    $dbname = 'testdb';
    $conn = mysqli_connect($dbhost, $dbuser, $dbpass) or die('Error with MySQL connection');
    $_SESSION['conn'] = $conn;
    mysqli_query($conn, "SET NAMES 'utf8'");
    mysqli_select_db($conn, $dbname);

?>
    <form action="feeder.php" method="get">
      <label>選擇索引條件：</label>
      <select name="select">
        <option value="select-t">---</option>
        <option value="select-a">全部</option>
　      <option value="select-b">編號</option>
　      <option value="select-c">物種</option>
　      <option value="select-d">性別</option>
      </select>
      <input type="text" name="input" >
       <input type="submit" name="new" value="搜尋">
    </form>

<?php
  if(isset($_GET['new'])){
      if($_GET['select'] == "select-a"){
        $sql = "SELECT * FROM animal  where E_id = '{$_SESSION["id"]}'";
        //$sql = "SELECT * FROM customer where account = '$username' and password =  '$password';";
        $result = mysqli_query($conn, $sql) or die('請檢查輸入條件');
        echo $_SESSION[ "id" ];
        echo "<br>使用者 : ";
        echo $_SESSION["Name"],"<br><br>";
  
       
        while($row = mysqli_fetch_array($result)){
         //echo $row['A_id'], $row['E_id'],"<p>";
         $tmp = $row['A_id'];
         //$_SESSION["Aid"] = $tmp;//有問題 會給迴圈最後一次的值
         echo "<a href=Adetail.php?A_id=$tmp>".$row['A_id']."</a>";
         echo  "\t";
         echo  $row['species'],"   ", $row['sex'], "   " , $row['E_id'], "\t", "<p>";
         echo "<br>";
        }
      }
      else if($_GET['select'] == "select-b") {
          if(!empty($_GET['input'])) {
            $sql = "SELECT * FROM animal  where A_id = '{$_GET['input']}'";
            $result = mysqli_query($conn, $sql) or die('請檢查輸入條件');
            echo "條件---動物編號","<p>";
            echo $_SESSION[ "id" ];
            echo "<br>使用者 : ";
            echo $_SESSION["Name"],"<br><br>";
      
           
            while($row = mysqli_fetch_array($result)){ 
                //echo $row['A_id'], $row['E_id'],"<p>";
                $tmp = $row['A_id'];
                //$_SESSION["Aid"] = $tmp;//有問題 會給迴圈最後一次的值
                echo "<tr>";
                echo "<a href=Adetail.php?A_id=$tmp>".$row['A_id']."</a>";
                echo  "\t";
                echo  "<td>",$row['species'],"</td><td>  ", $row['sex'],"</td>", "<p>";
                echo "</tr><br>";
            }
          }
          else{
            echo "請輸入條件";
          }
      } 
      else if($_GET['select'] == "select-c"){
        if(!empty($_GET['input'])){
          $sql = "SELECT * FROM animal  where species = '{$_GET['input']}'";
          $result = mysqli_query($conn, $sql) or die('MySQL query error');
          echo "條件---動物物種","<p>";
          echo $_SESSION[ "id" ];
          echo "<br>使用者 : ";
          echo $_SESSION["Name"],"<br><br>";
    
         
          while($row = mysqli_fetch_array($result)){ 
              //echo $row['A_id'], $row['E_id'],"<p>";
              $tmp = $row['A_id'];
              //$_SESSION["Aid"] = $tmp;//有問題 會給迴圈最後一次的值
              echo "<a href=Adetail.php?A_id=$tmp>".$row['A_id']."</a>";
              echo  "\t";
              echo  $row['species'],"   ", $row['sex'], "   " ,"\t", "<p>";
              echo "<br>";
          }
        }
        else{
          echo "請檢查輸入條件";
        }
      }
      else if($_GET['select'] == "select-d"){
        if(!empty($_GET['input'])){
          $sql = "SELECT * FROM animal  where sex = '{$_GET['input']}'";
          $result = mysqli_query($conn, $sql) or die('請檢查輸入條件');
          echo "條件---動物性別","<p>";
          echo $_SESSION[ "id" ];
          echo "<br>使用者 : ";
          echo $_SESSION["Name"],"<br><br>";
    
         
          while($row = mysqli_fetch_array($result)){ 
              //echo $row['A_id'], $row['E_id'],"<p>";
              $tmp = $row['A_id'];
              //$_SESSION["Aid"] = $tmp;//有問題 會給迴圈最後一次的值
              echo "<a href=Adetail.php?A_id=$tmp>".$row['A_id']."</a>";
              echo  "\t";
              echo  $row['species'],    $row['sex'], "   " ,  "\t", "<p>";
              echo "<br>";
          }
        }
        else{
          echo "請輸入條件";
        }
      }
  }
  else{
    echo "全部資訊";
      $sql = "SELECT * FROM animal  where E_id = '{$_SESSION["id"]}'";
      //$sql = "SELECT * FROM customer where account = '$username' and password =  '$password';";
      $result = mysqli_query($conn, $sql) or die('錯誤');
      echo $_SESSION[ "id" ];
      echo "<br>使用者 : ";
      echo $_SESSION["Name"],"<br><br>";

     
      while($row = mysqli_fetch_array($result)){
       //echo $row['A_id'], $row['E_id'],"<p>";
       $tmp = $row['A_id'];
       //$_SESSION["Aid"] = $tmp;//有問題 會給迴圈最後一次的值
       echo "<a href=Adetail.php?A_id=$tmp>".$row['A_id']."</a>";
       echo  "\t";
       echo  $row['species'],"   ", $row['sex'], "   " ,  "\t", "<p>";
       echo "<br>";
      }
  }
    ?>

    <form action="feeder.php" method="post">
    
    <label>編號：</label><input type="text" name="newid" ><br>
    <label>物種：</label><input type="text" name="newspecies" ><br>
    <label>性別：</label><input type="text" name="newsex" ><br>
    <input type="submit" name="new" value="新增">

    </form>
    
<?php
  if(!empty($_POST["new"])){
      if($_SERVER["REQUEST_METHOD"] == "POST"){
          if (empty($_POST["newid"])) {
            echo "<br>",'。請輸入編號', "<br>";
          } else {
            $newid = test_input($_POST["newid"]);
          }
        
          if (empty($_POST["newspecies"])) {
            echo "。請輸入物種", "<br>";
          } else {
            $newspecies = test_input($_POST["newspecies"]);
          }

          if (empty($_POST["newsex"])) {
            echo "。請輸入性別", "<br>";
          } else {
            $newsex = test_input($_POST["newsex"]);
          }
      }
   }
        
          function test_input($data) {
            $data = trim($data);
            $data = stripslashes($data);
            $data = htmlspecialchars($data);
            return $data;
          }

      //echo $_SESSION[ "password" ];
      //$conn = $_SESSION['conn'];
      $id = $_SESSION[ "id" ];

      if(isset($newid) AND isset($newspecies) AND isset($newsex)){
        echo $newid,"   ";
        echo $newspecies,"   ";
        echo $newsex,"   ";
        //$sql = "UPDATE animal SET A_id='Doe',species='$_GET['newspecies']', newsex='$_GET['newsex']' WHERE id=2";
        $sql = "INSERT INTO animal(A_id,species,sex,E_id) VALUES('$newid','$newspecies','$newsex','$id')";
        $result = mysqli_query($conn, $sql);
        if($result){
          echo "新增成功!<br>";
        }else{
          echo'新增失敗,請確認資料是否已經存在<br>';
        }
        
      } 
      else {
        if(!empty($_POST["new"])){
          if($_SERVER["REQUEST_METHOD"] == "POST"){
           echo "error<br>";
          }
        }
      }

 /*     $sql = "SELECT * FROM animal  where E_id = '{$_SESSION["id"]}'";
      //$sql = "SELECT * FROM customer where account = '$username' and password =  '$password';";
      $result = mysqli_query($conn, $sql) or die('MySQL query error');
      echo $_SESSION[ "id" ];
      echo "<br>使用者 : ";
      echo $_SESSION["Name"],"<br><br>";

     
      while($row = mysqli_fetch_array($result)){
       //echo $row['A_id'], $row['E_id'],"<p>";
       $tmp = $row['A_id'];
       //$_SESSION["Aid"] = $tmp;//有問題 會給迴圈最後一次的值
       echo "<a href=Adetail.php?A_id=$tmp>".$row['A_id']."</a>";
       echo  "\t";
       echo  $row['species'],"   ", $row['sex'], "   " , $row['E_id'], "\t", "<p>";
       echo "<br>";
      }*/
      
      
    ?>